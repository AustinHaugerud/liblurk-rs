...
impl Extractor {
  ...
  pub fn extract<F>(&self, stream : &mut F) -> Result<Vec<u8>, IOError> where F: Read {
    let mut result : Vec<u8> = vec![];

    for node in self.pattern_nodes.iter() {
      let data : Vec<u8> = match node {
        &ExtractNode::ByteChunk(size) => {
          self.extract_byte_chunk(stream, size)?
        },
        &ExtractNode::FixedStringChunk => {
          self.extract_fixed_string_chunk(stream)?
        },
        &ExtractNode::VarStringChunk =>  {
          self.extract_var_string_chunk(stream)?
        },
  ...
  fn extract_byte_chunk<F>(&self, stream : &mut F, n : u16) -> Result<Vec<u8>, IOError> where F: Read {
    let mut buffer : Vec<u8> = vec![0u8; n as usize];
    stream.read_exact(&mut buffer)?;
    Ok(buffer)
  }

  fn extract_fixed_string_chunk<F>(&self, stream : &mut F) -> Result<Vec<u8>, IOError> where F: Read {
    let mut buffer : Vec<u8> = vec![0u8; LURK_FIXED_STRING_SIZE as usize];
    stream.read_exact(&mut buffer)?;
    Ok(buffer)
  }

  fn extract_var_string_chunk<F>(&self, stream : &mut F) -> Result<Vec<u8>, IOError> where F: Read {
  ...
  fn extract_fractured_var_string_chunk<F>(&self, stream : &mut F, n : u16) -> Result<Vec<u8>, IOError> where F: Read {
